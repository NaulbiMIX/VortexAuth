...
public class AdvancedLicense {
	...
	public AdvancedLicense(String licenseKey, Plugin plugin) {
		this.licenseKey = licenseKey;
		this.plugin = plugin;
		this.validationServer = "https://wortexmine.ru/verify.php";
	}
	...
	public boolean register() {
		log(0, "[hAuth] Проверка лицензии...");
		ValidationType vt = isValid();
		if (vt == ValidationType.VALID) {
			log(1, "[hAuth] Проверка пройдена!");
			return true;
		} else {
			log(1, "[hAuth] Лизензия не прошла проверку!");
			log(1, "[hAuth] Разработчик данного проекта пидор который спиздил плагин, иди нахуй мудак!");
			log(1, "[hAuth] Плагин выключается...");
	...
	public ValidationType isValid() {
		...
		try {
			...
			if (response.startsWith("<")) {
				log(1, "Сервер Лицензии не вернул ответа.");
				log(1, "Обратитесь к https://vk.com/sayhe");
				log(1,
